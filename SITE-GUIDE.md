...
- `robots.txt` + `sitemap.xml` — `sitemap.xml` is generated by `scripts/build_sitemap.py`
  from the catalog (every character / body / series / family URL, not just bare templates);
  re-run it after the catalog changes. The param-driven detail pages self-canonicalize at
  runtime (`ZX.mountNav`) so each one is its own canonical, not a duplicate of the template.
